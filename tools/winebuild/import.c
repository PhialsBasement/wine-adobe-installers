...
/* The delay-load IAT must be writable, so it cannot live in .idata with binutils >= 2.44.
 * Use the MSVC-style .didat$N sections: binutils ld places .didat$5 in a writable .didat
 * output section and keeps the same archive-member ordering guarantees as .idata$N. */
