...
static HRESULT create_local_file_reference(IDWriteFactory7 *factory, const WCHAR *filename, IDWriteFontFile **file)
{
    ...
    if (!wcschr(filename, '\\'))
    {
        /* Resolve against C:\windows\fonts, falling back to Wine's build/data font dir
         * when the file isn't physically present in the prefix (bundled fonts are loaded
         * from the data dir, not copied into the prefix). Keeps the DWrite system collection
         * consistent with GDI so system/last-resort font lookups succeed on a fresh prefix. */
        WCHAR *fullpathW = dwrite_resolve_font_path(filename);
